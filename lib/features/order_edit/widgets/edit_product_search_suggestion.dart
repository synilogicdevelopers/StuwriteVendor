...
import 'package:stuwrite_vendor/features/order_edit/widgets/edit%20_product_bottomsheet.dart';
...
class EditProductSearchSuggestion extends StatefulWidget {
  final int? id;
  const EditProductSearchSuggestion({super.key,  this.id});
  @override
  State<EditProductSearchSuggestion> createState() => _EditProductSearchSuggestionState();
}

class _EditProductSearchSuggestionState extends State<EditProductSearchSuggestion> with WidgetsBindingObserver {
              ...
              child: Autocomplete<Product>(
                ...
                optionsBuilder: (TextEditingValue textEditingValue) async {
                  ...
                  await clearanceController.getSearchProductList(
                    Provider.of<ProfileController>(context, listen: false).userId.toString(), widget.id.toString(),
                    1,  'en', textEditingValue.text, reload: false
                  );
                  ...
                  if (textEditingValue.text.isEmpty || clearanceController.sellerProductModel?.products == null) {
                    ...
                    Iterable<Product> matchingProducts = clearanceController.sellerProductModel!.products!.where(
                      (product) => (product.name!.toLowerCase().contains(textEditingValue.text.toLowerCase()) &&
                        (!(clearanceController.cartList.any((item) => item.product!.id! == product.id  && !(product.variation != null && product.variation!.isNotEmpty)))) &&
                            ...
                            return InkWell(
                              onTap: () async {
                                ...
                                if(product.productType == 'digital') {
                                  showCustomSnackBarWidget(getTranslated('digital_product_not_allowed', context), context, sanckBarType: SnackBarType.warning);
                                } else if (product.productType == 'physical' && product.variation != null && product.variation!.isNotEmpty) {
                                  showModalBottomSheet(context: context, isScrollControlled: true,
                                    backgroundColor: Theme.of(context).primaryColor.withValues(alpha:0),
                                    builder: (con) => EditProductCartBottomSheetWidget(
                                      product: product,
                                      orderId: widget.id,
                                    )
                                  );
                                } else {
                                  clearanceController.addToCart(product);

                                  showDialog(context: context, builder: (ctx)  => const CustomLoaderWidget(), barrierDismissible: false);
                                  await Provider.of<OrderEditController>(context, listen: false).editOrderValidation(widget.id ?? 0);
                              ...
                              child: Padding(
                                ...
                                child: Opacity(
                                  opacity: product.productType == 'digital' ? 0.5 : 1.0,
                                  child: Container(
                                    padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
                                              ...
                                              Row(
                                                children: [
                                                  ...
                                                  Text(PriceConverter.convertPrice(context, product.unitPrice),
                                                    ...
                                                    style: robotoRegular.copyWith(color: Theme.of(context).colorScheme.primary.withValues(alpha: 0.5),
